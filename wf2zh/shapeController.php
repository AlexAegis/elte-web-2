...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        case 'list':
            ...
            $errors = array();
            $other = array();
            $shapes = array_values(R::find('alakzatok'
            , '(not :kedvencSearch) or kedvenc = :kedvenc'
            , ['kedvencSearch' => isset($_GET['parameter']['kedvenc'])
                    , 'kedvenc' => isset($_GET['parameter']['kedvenc']) ? ($_GET['parameter']['kedvenc'] == 'true' ? 1 : 0) : 1]));
